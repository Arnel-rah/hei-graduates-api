...
VALUES ('admin@hei.school', 'admin123', 'ADMIN'),
...
VALUES ('Promotion K', '2025-01-01'::timestamp),
       ('Promotion L', '2026-01-01'::timestamp);


INSERT INTO group_ (ref, promotion_id)
SELECT 'K2', id
FROM promotion
WHERE label = 'Promotion K'
UNION ALL
SELECT 'K3', id
FROM promotion
WHERE label = 'Promotion K'
...
SELECT 'Andriamihaja', 'Jean', 'STD001', a.id, p.id
FROM account a
         CROSS JOIN promotion p
WHERE a.email = 'student1@hei.school'
  AND p.label = 'Promotion K'
UNION ALL
SELECT 'Rakotomalala', 'Paul', 'STD002', a.id, p.id
FROM account a
         CROSS JOIN promotion p
WHERE a.email = 'student2@hei.school'
  AND p.label = 'Promotion K'
UNION ALL
SELECT 'Randria', 'Marie', 'STD003', a.id, p.id
FROM account a
         CROSS JOIN promotion p
...
SELECT '2026-03-15 09:00:00'::timestamp, 2.00, ca.id
FROM course_assignment ca
         JOIN course c ON c.id = ca.course_id
WHERE c.ref = 'PROG3'
UNION ALL
SELECT '2026-03-20 09:00:00'::timestamp, 2.00, ca.id
FROM course_assignment ca
         JOIN course c ON c.id = ca.course_id
WHERE c.ref = 'DB1';

INSERT INTO grade (value, student_id, exam_id)
SELECT 15.50, s.id, e.id
FROM student s
         JOIN account a ON a.id = s.account_id
         JOIN exam e ON e.date_exam = '2026-03-15 09:00:00'::timestamp
         JOIN course_assignment ca ON ca.id = e.course_assignment_id
         JOIN course c ON c.id = ca.course_id
WHERE a.email = 'student1@hei.school'
  AND c.ref = 'PROG3'
UNION ALL
SELECT 13.00, s.id, e.id
FROM student s
         JOIN account a ON a.id = s.account_id
         JOIN exam e ON e.date_exam = '2026-03-15 09:00:00'::timestamp
         JOIN course_assignment ca ON ca.id = e.course_assignment_id
...
WHERE a.email = 'student2@hei.school'
  AND c.ref = 'PROG3'
UNION ALL
SELECT 17.00, s.id, e.id
FROM student s
         JOIN account a ON a.id = s.account_id
         JOIN exam e ON e.date_exam = '2026-03-20 09:00:00'::timestamp
         JOIN course_assignment ca ON ca.id = e.course_assignment_id
         JOIN course c ON c.id = ca.course_id
WHERE a.email = 'student1@hei.school'
  AND c.ref = 'DB1';
...
SELECT '2026-01-10'::timestamp, NULL, s.id, g.id
FROM student s
         JOIN account a ON a.id = s.account_id
         JOIN group_ g ON g.ref = 'K2'
WHERE a.email IN ('student1@hei.school', 'student2@hei.school');